...
SampleWindow::SampleWindow(int width, int height, std::shared_ptr<App> app)
{
    ...
    winrt::check_bool(CreateWindowExW(exStyle, ClassName.c_str(), L"AimBot by Yuan", style,
    ...
    m_statusBarHwnd = CreateWindowEx(
        0,
        STATUSCLASSNAME,
        L"123",
...
LRESULT SampleWindow::MessageHandler(UINT const message, WPARAM const wparam, LPARAM const lparam)
{
        ...
        case CBN_SELCHANGE:
            {
                ...
                if (hwnd == m_windowComboBox)
                {
                    ...
                    m_app->SetStatusBarHwnd(m_statusBarHwnd);
                    if (item != nullptr)
                    {
    ...
    case WM_SIZE:
        // SendMessage(labelHwnd, WM_SETTEXT, 0, (LPARAM)L"aaa");
        SendMessage(m_statusBarHwnd, WM_SIZE, 0, 0);
    default:
...
void SampleWindow::CreateControls(HINSTANCE instance)
{
    ...
    SendMessageW(captureExcludeCheckBox, BM_SETCHECK, BST_CHECKED, 0);
...
{
    std::wstring titleText(L"AimBot by Yuan");
    if (!text.empty())
    {
        titleText += (L" - " + text);
    }
