    ...
    const cv::Size& inputSize = cv::Size(640, 640))
{
...
#ifdef _WIN32
    std::wstring w_modelPath = utils::stringToWstring(modelPath);
    ...
    inputNames.push_back(inputNamePtr.get());
...
void YOLODetector::preprocessing(cv::Mat& image, float*& blob, std::vector<int64_t>& inputTensorShape)
{
    ...
    utils::letterbox(resizedImage, resizedImage, this->inputImageShape,
        ...
        false, true, 32);
    /*
    utils::letterbox(resizedImage_gpu, resizedImage_gpu, this->inputImageShape,
        cv::Scalar(114, 114, 114), this->isDynamicInputShape,
        false, true, 32);
    */
...
void YOLODetector::preprocessing(cv::cuda::GpuMat& image, float*& blob, std::vector<int64_t>& inputTensorShape)
{
    cv::cuda::GpuMat resizedImage, floatImage;
    cv::cuda::cvtColor(image, resizedImage, cv::COLOR_BGRA2RGB);
    utils::letterbox(resizedImage, resizedImage, this->inputImageShape,
        cv::Scalar(114, 114, 114), this->isDynamicInputShape,
        false, true, 32);
    ...
    resizedImage.convertTo(floatImage, CV_32FC3, 1.f / 255.f);
    blob = new float[floatImage.cols * floatImage.rows * floatImage.channels()];
    cv::Size floatImageSize{ floatImage.cols, floatImage.rows };
    //  std::cout << floatImageSize<<std::endl;
    // hwc -> chw
    std::vector<cv::Mat> chw(floatImage.channels());
    for (size_t i = 0; i < floatImage.channels(); ++i)
    {
        chw[i] = cv::Mat(floatImageSize, CV_32FC1, blob + i * floatImageSize.width * floatImageSize.height);
    }
    //std::cout << floatImage << std::endl;
    cv::Mat flt_image;
    floatImage.download(flt_image);
...
std::vector<Detection> YOLODetector::detect(cv::cuda::GpuMat& image)
{
    float* blob = nullptr;
    std::vector<int64_t> inputTensorShape{ 1, 3, -1, -1 };
    this->preprocessing(image, blob, inputTensorShape);
