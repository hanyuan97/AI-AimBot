...
{
	auto copyMakeBorder_start = std::chrono::high_resolution_clock::now();
	...
	auto copyMakeBorder_end = std::chrono::high_resolution_clock::now();
	auto copyMakeBorder_time = std::chrono::duration_cast<std::chrono::milliseconds>(copyMakeBorder_end - copyMakeBorder_start).count();
	std::cout << "copyMakeBorder_time: " << copyMakeBorder_time << "ms" << std::endl;
...
{
	cv::Mat nchw, resizedImage;
	cv::cvtColor(image, resizedImage, cv::COLOR_BGRA2RGB);
	...
	auto cudamemcpy_start = std::chrono::high_resolution_clock::now();
	CHECK(cudaMemcpyAsync(
		this->device_ptrs[0],
		nchw.ptr<float>(),
	...
	auto cudamemcpy_end = std::chrono::high_resolution_clock::now();
	auto cudamemcpy_time = std::chrono::duration_cast<std::chrono::milliseconds>(cudamemcpy_end - cudamemcpy_start).count();
	std::cout << "cudamemcpy_time: " << cudamemcpy_time << "ms" << std::endl;
}
