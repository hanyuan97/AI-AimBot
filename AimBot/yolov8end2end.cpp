...
YOLOv8end2end::YOLOv8end2end(const std::string& engine_file_path)
{
...
void YOLOv8end2end::make_pipe(bool warmup)
{
...
void YOLOv8end2end::letterbox(const cv::Mat& image, cv::Mat& out, cv::Size& size)
{
...
void YOLOv8end2end::copy_from_Mat(const cv::Mat& image)
{
...
void YOLOv8end2end::copy_from_Mat(const cv::Mat& image, cv::Size& size)
{
	cv::Mat nchw;
	this->letterbox(
		image,
...
void YOLOv8end2end::postprocess(std::vector<Detection>& objs)
{
	objs.clear();
...
//
//void YOLOv8end2end::draw_objects(
//	const cv::Mat& image,
//	cv::Mat& res,
